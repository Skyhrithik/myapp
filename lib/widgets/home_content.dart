...
//   // qty per product (fast updates with ValueNotifier)
//   final Map<int, int> _qty = {}; // productId -> quantity
//   final Map<int, ValueNotifier<int>> _qtyVN = {};
//   ValueNotifier<int> _vnFor(Product p) =>
//       _qtyVN.putIfAbsent(p.id, () => ValueNotifier(_qty[p.id] ?? 0));
...
//       );
//     }
//     _categories = m.values.toList()..sort((a, b) => a.name.compareTo(b.name));
//   }
...
//   //             child: const Icon(Icons.category, color: Colors.white, size: 26),
//   //           ),
//   //           const SizedBox(height: 6),
//   //           ConstrainedBox(
//   //             constraints: const BoxConstraints(maxWidth: 96),
...
//               radius: 30,
//               backgroundColor: kPrimary,
//               // 🔙 use the dynamic icon we computed when building categories
//               child: Icon(c.icon, color: Colors.white, size: 28),
//             ),
//             const SizedBox(height: 6),
//             ConstrainedBox(
...
//       onInc: _inc,
//       onDec: _dec,
//       priceTextFn: _priceText,
//       maxPerItem: _maxPerItem,
//       // Optional visuals if you want:
//       // weightLabel: '48 g',
//       // deliveryMins: 8,
//     );
//   }
...
//                   (context, index) => _productCard(_visibleProducts[index]),
//                   childCount: _visibleProducts.length,
//                 ),
//               ),
//             ),

//           // Error banner (if we still show cached list)
//           if (_error != null && _products.isNotEmpty)
//             SliverToBoxAdapter(
//               child: Padding(
//                 padding: const EdgeInsets.symmetric(
...
//           return ModernProductCard(
//             product: p,
//             imgUrl: imgUrlBuilder,
//             qtyVN: getQtyVN,
//             onAddToCart: onAddToCart,
//             onInc: onInc,
//             onDec: onDec,
//             priceTextFn: (pp) => '₹ ${_priceDouble(pp).toStringAsFixed(2)}',
...
class _HomeContentState extends State<HomeContent>
  ...
  // keep a handle to CartProvider to listen for changes
  CartProvider? _cart;
  ...
  /// Attach/detach CartProvider listener and do an initial sync.
  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    final next = context.read<CartProvider>();
    if (_cart != next) {
      _cart?.removeListener(_syncQtyFromCart);
      _cart = next;
      _cart!.addListener(_syncQtyFromCart);
    }
    _syncQtyFromCart(); // initial sync when dependencies change
  }
  ...
  Future<void> _tryLoadFromCache() async {
    try {
      ...
      _syncQtyFromCart(); // reflect cart state on cached draw
    } catch (_) {}
  }
  ...
  Future<void> _loadProducts() async {
    try {
      ...
      _syncQtyFromCart(); // reflect cart state on fresh data
      // ignore: unawaited_futures
      _saveCache(items);
    } catch (e) {
      if (!mounted) return;
  ...
  /// Keep local qty + notifiers in sync with CartProvider (source of truth).
  void _syncQtyFromCart() {
    final cart = _cart ?? context.read<CartProvider>();
    // cart.lines keys are String productIds; convert to int -> qty
    final Map<int, int> cartQty = {};
    for (final entry in cart.lines.entries) {
      final pid = int.tryParse(entry.key);
      if (pid != null) cartQty[pid] = entry.value.qty;
    }
    // Update all known products
    for (final p in _products) {
      final q = cartQty[p.id] ?? 0;
      _qty[p.id] = q;
      _vnFor(p).value = q; // rebuild only qty rows efficiently
    }
  }
  ...
  List<Product> get _visibleProducts {
    ...
    // optional: sort so in-stock items appear first
    list.sort((a, b) {
      final aIn = a.stock > 0 ? 1 : 0;
      final bIn = b.stock > 0 ? 1 : 0;
      // in-stock first; keep original order among equals
